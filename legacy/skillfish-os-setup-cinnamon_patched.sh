#!/usr/bin/env bash
# Skillfish OS setup for AMD BC-250 (Debian "forky" netinstall → Cinnamon minimale + autologin + gaming stack)
#
# FASE 1:
...
#   - Nessun reboot automatico finale (verrà richiesto reboot manuale)
#
# Log:
...
apt_install_nr() {
  # Install packages without recommends (keeps Cinnamon very minimal)
  local pkgs=("$@")
  DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends "${pkgs[@]}" || {
    warn "apt (no-recommends) fallito per: ${pkgs[*]} (provo fix deps e ritento)"
    DEBIAN_FRONTEND=noninteractive apt-get -f install -y || true
    DEBIAN_FRONTEND=noninteractive apt-get install -y --no-install-recommends "${pkgs[@]}" || true
  }
}

preseed_lightdm_default_dm() {
  # Avoid interactive prompt when installing a display manager
  if ! command -v debconf-set-selections >/dev/null 2>&1; then
    apt_install debconf-utils || true
  fi
...
configure_lightdm_autologin() {
  ...
  local conf_file="$conf_dir/50-autologin.conf"
  mkdir -p "$conf_dir"

  # Ensure autologin group exists and user is in it (common LightDM convention)
  getent group autologin >/dev/null 2>&1 || groupadd -r autologin || true
  usermod -aG autologin "$u" 2>/dev/null || true
...
install_minimal_cinnamon_desktop() {
  # Netinstall → minimal Cinnamon (no LibreOffice/Thunderbird), with LightDM + NetworkManager + autologin
  msg "Installo desktop Cinnamon MINIMALE (cinnamon-core) + LightDM + autologin…"
  local u; u="$(get_target_user)"
  if [[ -z "$u" ]]; then
    warn "Impossibile determinare l'utente target per autologin. Installo comunque Cinnamon senza autologin."
  fi

  preseed_lightdm_default_dm

  # Core system bits for a usable but minimal desktop
  apt_update

  # Xorg + amdgpu driver + minimal Cinnamon core
  # cinnamon-core is the minimal Cinnamon metapackage. (Debian: "Install this if you want a minimal cinnamon desktop")
  # We purposely avoid task-cinnamon-desktop / cinnamon-desktop-environment to keep the system lean.
  apt_install_nr \
    xorg \
  ...
  if [[ -n "$u" ]]; then
    configure_lightdm_autologin "$u"
  fi

  msg "Cinnamon minimale installato. (Se sei ancora in console, dopo il reboot partirà l'ambiente grafico.)"
}

install_timeshift_gui() {
  msg "Installo Timeshift (GUI) per snapshot/ripristino…"
  apt_update
  apt_install timeshift || true
}
...
ensure_log() {
  ...
  # The log path is persisted across reboot (phase1 → phase2).
  # On netinstall the "start dir" might not exist (e.g. Desktop folders not created yet),
  # so we always fall back to /var/log if needed.
  if [[ -f "$LOGPATH_FILE" ]]; then
    LOG_FILE="$(cat "$LOGPATH_FILE" 2>/dev/null || true)"
  fi
  ...
  if [[ -z "${LOG_FILE:-}" ]]; then
    ...
    if [[ -n "${start_dir:-}" && -d "$start_dir" ]]; then
      LOG_FILE="$start_dir/skillfish-os-setup.log"
    else
      LOG_FILE="/var/log/skillfish-os-setup.log"
    fi
...
# -----------------------------
# Bluetooth + Wi-Fi + LAN browsing (Samba/NFS)
# -----------------------------
install_network_support() {
  msg "Supporto Bluetooth, Wi-Fi e navigazione LAN (Samba/NFS)…"
...
main() {
  ...
  # Netinstall: install minimal Cinnamon BEFORE switching kernel,
  # so that after reboot you have a GUI and the log tail autostart can work.
  local codename; codename="$(detect_codename)"
  if [[ "$codename" == "forky" ]]; then
    disable_suite_sources "trixie"
  fi
